...
const getDeliveryByOrderIDQuery = `
	SELECT
		id,
		order_id,
		courier_id,
...
func GetDeliveryByOrderID(ctx context.Context, pool *pgxpool.Pool, orderID uuid.UUID) (model.Delivery, error) {
	var delivery model.Delivery
	err := pool.QueryRow(
		ctx,
		getDeliveryByOrderIDQuery,
		orderID,
	).Scan(
		&delivery.ID,
		&delivery.OrderID,
